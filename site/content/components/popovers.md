...
- Popovers rely on the third party library [Popper](https://popper.js.org/) for positioning.
...
const popoverList = [...popoverTriggerList].map(async popoverTriggerEl => await openFrontend.Popover.then(component => new component(popoverTriggerEl)))
```
...
```js
const popover = await openFrontend.Popover.then(component => component.getOrCreateInstance('.example-popover', {
  container: 'body'
}))
```
...
```js
const popover = await openFrontend.Popover.then(component => component.getOrCreateInstance('.popover-dismiss', {
  trigger: 'focus'
}))
```
...
const popover = await openFrontend.Popover.then(component => component.getOrCreateInstance(exampleEl, options))
```
...
| --- | --- | --- | --- |
| `allowList` | object | [Default value]({{< docsref "/getting-started/javascript-api#sanitizer" >}}) | Object which contains allowed attributes and tags. |
...
```js
const popover = await openFrontend.Popover.then(component => component.getOrCreateInstance(exampleEl, {
