...
```js
const collapse = await openFrontend.Collapse.then(component => component.getOrCreateInstance('#myCollapse', {
  toggle: false
}))
```
...
| `getInstance` | Static method which allows you to get the collapse instance associated to a DOM element. |
